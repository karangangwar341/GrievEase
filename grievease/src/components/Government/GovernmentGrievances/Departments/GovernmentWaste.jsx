...
const GovernmentWaste = () => {
  const [grievanceData, setGrievanceData] = useState([]);

  const fetchGrievanceData = async () => {
    const userCollections = collection(database, "grievances");
    const q =query(userCollections, where('status', 'in', ['pending', 'inprocess']), where('department','==','Waste'));
  ...
  return (
    <div>
      <div className="card-headerG text-3xl font-bold text-orange-900 mb-4 items-center justify-center' text-center">Grievances Pending at Waste DepartMent</div>
...
export default GovernmentWaste;
